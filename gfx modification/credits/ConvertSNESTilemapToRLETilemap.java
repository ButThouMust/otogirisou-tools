...
public class ConvertSNESTilemapToRLETilemap {
    ...
    private static String getOutputFilename(String inputFilename) throws IOException {
        // first, get rid of the file extension
        int periodIndex = inputFilename.lastIndexOf('.');
        if (periodIndex == -1) {
            String errorMessage = "Tilemap file should have a proper file extension.";
            throw new IOException(errorMessage);
        }
        String inputFilenameNoExt = inputFilename.substring(0, periodIndex);

        // then remove the "SNES" in the file name, and prefix it with "RLE"
        String removeSNES[] = inputFilenameNoExt.split("SNES ");
        return "RLE " + removeSNES[0] + removeSNES[1] + ".bin";

        // return inputFilename.substring(0, periodIndex) + " - for RLE compressor.bin";
    }

    public static void convertToDumpTilemap(String filename, int width, int height) throws IOException {
    ...
    public static void main(String args[]) throws IOException {
        if (args.length != 3) {
            System.out.println("Sample usage: java ConvertDumpToDumpTilemap dumped_tilemap.bin tilemap_width tilemap_height");
            return;
        }
